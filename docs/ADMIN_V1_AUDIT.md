...
| **TD-3** | **Bank details in `localStorage`** | `exhibitions/IssueDocumentPage.tsx` (the path this row first gave was one folder too shallow) `readBank`/`saveBank` keep the payee account holder, bank, card and IBAN per-device. A second admin issuing an invoice gets blanks; a cleared browser loses them. ~~Belongs under `theme.*` (App Design) or its own endpoint.~~ ✅ **Fixed 2026-09-22 — and NOT under `theme.*`, which was the wrong half of this row's own suggestion:** `GET /api/app-theme/` is `AllowAny` (the collector app reads it before sign-in), so a card number and an IBAN in it would be published to anyone who can reach the API — strictly worse than the bug. Instead the page seeds from **the most recent issued invoice's `Document.fields.bank`**, which every invoice already stores server-side: shared between admins, durable across a cleared browser, no new endpoint and nothing published. The device copy stays as a head start only. There is still nowhere to *edit* the studio's details except by issuing an invoice (the old panel's behaviour too) — a proper settings home is one small endpoint, recorded as **G-DOC-1**. |
...
| **G-HEALTH-1** | Build the eight Data Health counts that are real catalogue counts? | Yes — but as **one** aggregate on the existing data-health endpoint, not eight client reads. ✅ *Built 2026-09-22, client-side: **four** of the eight, for one extra read (two come free from the report the desk already loads). The other four are backend gaps, now named — **G-HEALTH-2** (no `source_type` on an artwork, so Gallery/Dealer/Artist-Sourced cannot be counted), **G-HEALTH-3** (no deleted-row count), **G-HEALTH-4** (no `created_after` filter, so "Recently Added" cannot be asked). The aggregate-endpoint recommendation still stands and would close all three.* |
| **G-CLUB-2** | Collector Club's 3-tile strip? | The G-4 answer: the answerable tiles only, or none — "private auctions" has no backend. ✅ *Built 2026-09-22: two of three, "Private auctions" absent (G-CLUB-3).* |
| **G-MEMB-1** | Memberships' 4-tile strip? | Two of the four are answerable; Premium is not, because `plan` is the collector tier set. ✅ *Built 2026-09-22: Total members + Active. Premium is a vocabulary this backend does not have; "Expiring ≤ 7d" needs one `expires_before=` filter — **G-MEMB-7**, and the one of the four worth raising, because it is the only one that drives an action.* |
