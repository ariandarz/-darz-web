...
| Id | Gap | What the frontend does |
|---|---|---|
| **G-P24-1** | **No selection name.** The old chip labels itself from the selection's own name, falling back to the generic string only when there isn't one (`app.html:3458`: `return (s[0]&&s[0].name)||'Curated for You'`). Grants have no name, so the label can only ever be the fallback. | The chip always reads **"Curated for You"**. This is a real (small) loss of fidelity: a gallery that named a selection cannot have the collector see that name. Owner decision D6. |
