...
| **G-DOC-1** | *(see §5)* no document ref on `RequestMessage`. | Share by link — the detail's **Copy link** (signed URL for private, stable for public) is that resolution live. |
