...
/** A small search-and-pick control: type → top matches → add; picked items are
 * removable chips. Used for both works and collectors. */
function Picker({
  label,
  placeholder,
  picked,
  ...
  const add = (item: PickItem) => {
    if (!picked.some((p) => p.id === item.id)) onChange([...picked, item]);
    setQ('');
    setHits([]);
  };
